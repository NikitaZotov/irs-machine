  ...
  *storage = irs_mem_new(irs_storage, 1);
  {
    ...
    static irs_char const * term_offsets = "term_offsets.irsdb";
    _irs_init_db_path(path, term_offsets, &(*storage)->term_offsets_path);
  }
...
irs_storage_status irs_storage_shutdown(irs_storage * storage)
{
...

  {
    ...
    irs_mem_free(storage->term_offsets_path);
...
{
  if (irs_list_init(document_offset_map) == IRS_FALSE)
  ...
  while (irs_iterator_next(it))
  {
    ...
    {
      irs_pair * pair;
      irs_uint64 * doc_offset = irs_mem_new(irs_uint64, 1);
      *doc_offset = storage->documents_size;
      irs_pair_initialize(&pair, document, doc_offset);
    ...
    // save document in db
    {
      ...
      storage->documents_size += written_bytes;
    ...
    irs_char * term, irs_list * document_offset_map, irs_list ** doc_with_term_offsets)
{
  ...
  while (irs_iterator_next(doc_it))
  {
    ...
    if (irs_str_find(document_to_find, term) == IRS_FALSE)
      continue;
...
{
  irs_float const reverse_frequency = logf((irs_float)documents_size / (irs_float)doc_with_term_offsets_size);
...
irs_float _irs_storage_calculate_term_frequency(irs_char * term, irs_list * document_terms)
{
  ...
  while (irs_iterator_next(it))
  {
    ...
    if (irs_str_cmp(term, other_term) == IRS_TRUE)
      ++count_in_document;
  }
  irs_iterator_destroy(it);

  irs_float const frequency = (irs_float)(count_in_document) / (irs_float)document_terms->size;
  return frequency;
}
...
irs_dictionary * _irs_storage_assign_terms_in_documents(irs_list * document_offset_map)
{
  ...
  while (irs_iterator_next(doc_it))
  {
    ...
    while (irs_iterator_next(term_it))
    {
      ...
      irs_uint64 doc_with_term_offsets_size;
      {
...
void _irs_storage_pass_terms_doc_significancy_dictionary(irs_dictionary_node * node, void ** arguments)
{
  ...
  *terms_offset_ptr = storage->terms_size;
  ...
  irs_dictionary_append(storage->term_offset_dictionary, term, term_size, term);
    ...
    irs_storage * storage, irs_uint64 const term_offset, irs_list ** documents_with_significancies)
{
  ...
  irs_uint64 read_bytes = 0;
  {
    irs_uint64 term_size;
    if (irs_io_channel_read_chars(terms_channel, (irs_char *)&term_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
    {
      irs_io_channel_shutdown(terms_channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(terms_channel);
      return IRS_STORAGE_READ_ERROR;
    }

    irs_char * term = irs_mem_new(irs_char, term_size + 1);
    if (irs_io_channel_read_chars(terms_channel, (irs_char *)term, term_size, &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || term_size != read_bytes)
    {
      irs_io_channel_shutdown(terms_channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(terms_channel);
      return IRS_STORAGE_READ_ERROR;
    ...
    if (irs_io_channel_read_chars(
            terms_channel, (irs_char *)&docs_with_term_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
    {
      irs_io_channel_shutdown(terms_channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(terms_channel);
  ...
  for (irs_uint64 i = 0; i < docs_with_term_size; ++i)
  {
    ...
    *doc_offset_ptr = doc_offset;
    ...
    irs_io_channel_seek(documents_channel, doc_offset, IRS_IO_SEEK_SET, NULL_PTR);
    {
      irs_uint64 doc_size;
      if (irs_io_channel_read_chars(
        documents_channel, (irs_char *)&doc_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
      {
      ...
      if (irs_io_channel_read_chars(documents_channel, (irs_char *)document, doc_size, &read_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || doc_size != read_bytes)
      {
        irs_io_channel_shutdown(documents_channel, IRS_TRUE, NULL_PTR);
        irs_mem_free(documents_channel);
        return IRS_STORAGE_READ_ERROR;
    ...
    if (irs_io_channel_read_chars(
      terms_channel, (irs_char *)&significancy_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
    {
      irs_io_channel_shutdown(terms_channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(terms_channel);
    ...
    if (irs_io_channel_read_chars(terms_channel, (irs_char *)significancy, significancy_size, &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || significancy_size != read_bytes)
    {
      irs_io_channel_shutdown(terms_channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(terms_channel);
      return IRS_STORAGE_READ_ERROR;
...
void _irs_storage_read_term_offsets(irs_storage * storage, irs_io_channel * channel)
{
  // save term in db
  irs_uint64 read_bytes = 0;
  while (IRS_TRUE)
  {
    irs_uint64 term_size;
    if (irs_io_channel_read_chars(channel, (irs_char *)&term_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
      break;

    irs_char * term = irs_mem_new(irs_char, 1);
    if (irs_io_channel_read_chars(channel, (irs_char *)term, term_size, &read_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || term_size != read_bytes)
      break;

    irs_uint64 * term_offset = irs_mem_new(irs_uint64, 1);
    if (irs_io_channel_read_chars(channel, (irs_char *)term_offset, sizeof(irs_uint64), &read_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
      break;

    irs_dictionary_append(storage->term_offset_dictionary, term, term_size, term_offset);
    irs_dictionary_append(storage->term_offset_dictionary, term, term_size, term);
  }
}
...
{
  irs_io_channel * term_offsets_channel = irs_io_new_channel(storage->term_offsets_path, "r", NULL_PTR);
  if (term_offsets_channel == NULL_PTR)
    return IRS_STORAGE_OK;

  irs_io_channel_set_encoding(term_offsets_channel, NULL_PTR, NULL_PTR);

  irs_uint64 read_bytes = 0;
  if (irs_io_channel_read_chars(term_offsets_channel, (irs_char *)&storage->terms_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
  {
    irs_io_channel_shutdown(term_offsets_channel, IRS_TRUE, NULL_PTR);
    irs_mem_free(term_offsets_channel);
    return IRS_STORAGE_READ_ERROR;
  }

  if (irs_io_channel_read_chars(term_offsets_channel, (irs_char *)&storage->documents_size, sizeof(irs_uint64), &read_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != read_bytes)
  {
    irs_io_channel_shutdown(term_offsets_channel, IRS_TRUE, NULL_PTR);
    irs_mem_free(term_offsets_channel);
  ...
  _irs_storage_read_term_offsets(storage, term_offsets_channel);
...
void _irs_storage_write_term_offsets(irs_dictionary_node * node, void ** arguments)
{
  if (node->data_list == NULL_PTR)
    return;
  ...
  irs_iterator * list_it = irs_list_iterator(node->data_list);
  irs_iterator_next(list_it);
  irs_uint64 * const term_offset = irs_iterator_get(list_it);
  ...
  irs_io_channel * channel = arguments[0];
  ...
  irs_uint64 written_bytes = 0;
  {
    ...
    if (irs_io_channel_write_chars(channel, (irs_char *)term_offset, sizeof(irs_uint64), &written_bytes, NULL_PTR)
        != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != written_bytes)
    {
      irs_io_channel_shutdown(channel, IRS_TRUE, NULL_PTR);
      irs_mem_free(channel);
...
{
  irs_io_channel * term_offsets_channel = irs_io_new_channel(storage->term_offsets_path, "w", NULL_PTR);
  irs_io_channel_set_encoding(term_offsets_channel, NULL_PTR, NULL_PTR);

  irs_uint64 written_bytes = 0;
  if (irs_io_channel_write_chars(term_offsets_channel, (irs_char *)&storage->terms_size, sizeof(irs_uint64), &written_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != written_bytes)
  {
    irs_io_channel_shutdown(term_offsets_channel, IRS_TRUE, NULL_PTR);
    irs_mem_free(term_offsets_channel);
  ...
  if (irs_io_channel_write_chars(term_offsets_channel, (irs_char *)&storage->documents_size, sizeof(irs_uint64), &written_bytes, NULL_PTR)
      != IRS_IO_STATUS_NORMAL || sizeof(irs_uint64) != written_bytes)
  {
    irs_io_channel_shutdown(term_offsets_channel, IRS_TRUE, NULL_PTR);
    irs_mem_free(term_offsets_channel);
  ...
  irs_dictionary_visit_down_nodes(
      storage->term_offset_dictionary, _irs_storage_write_term_offsets, (void **)&term_offsets_channel);
